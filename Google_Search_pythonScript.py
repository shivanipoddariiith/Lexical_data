...
def search_for(s):
	s1 = s.replace(' ','_')
	os.mkdir("/home/Madity/IIITH/Summer2014/"+s1)
	os.chdir("/home/Madity/IIITH/Summer2014/"+s1)
	page_text = requests.get(GOOGLE_BASE + urllib.quote(s+" pdf")).content
	parsed = lxml.html.fromstring(page_text)
	urls = parsed.cssselect('cite')
	l = len(urls)
	t = 0
	while t < l:
		if urls[t].text_content()[-4:] != ".pdf":
			t = t + 1
			continue
		temp = "wget "+urls[t].text_content()
		os.system(temp)
		t = t + 1
	os.chdir("/home/Madity/IIITH/Summer2014")
